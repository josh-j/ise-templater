...
**Inventory** — the list of machines. `inventory.yml` here. Ours has two
...
**Playbook** — a file of steps, run top to bottom. `export.yml`, `apply.yml`
and the rest. Each one is a *play*: a set of hosts plus the tasks to run
against them.
...
variable `ise_user` here". Variables live in `group_vars/all/`, which applies
to every host, and can be overridden on the command line.

**Template** — a file with `{{ ... }}` in it, filled in when it is used. The
`.j2` files under `templates/` are ISE objects in JSON with the deployment-
specific bits replaced by variables.

## Reading a run

```
TASK [sgt: export via ERS] *****************************************************
ok: [labb-ise-001]
```
...
2. `credentials.yml`, loaded by `tasks/load_credentials.yml`
3. `host_vars/<name>.yml` — settings for one appliance
4. `group_vars/all/*.yml` — settings for all of them
5. defaults written into the playbooks

So `credentials.yml` overrides `group_vars/`, and `-e` overrides everything.
That is the whole precedence story for this project; Ansible's full version has
more layers, and none of them are used here.

## Changing things

**Add an appliance**: copy a two-line host block in `inventory.yml`. Run
`./ise check`.

**Change what is collected**: `group_vars/all/02-families.yml` switches whole
families on and off. For one run, `./ise export --with portals`.

**Add a resource**: add an entry to `ise_catalog` in
`group_vars/all/03-catalog.yml`. Nothing else changes — the playbooks loop over
that list. [catalog.md](catalog.md) explains the keys.

**Change what a template contains**: the `.j2` files under `templates/` are
generated by `./ise templatize`, so edits there are lost on the next run. Edit
`site-templates/` instead, or the tokens in
`group_vars/all/04-templating.yml`.
